...
class GaussianDiffusion(nn.Module):
    ...
    def forward(self, img):
        ...
        t_value_min = torch.min(t)
        if t_value_min >1:
            predicted_min_noise_image = self.p_sample(noised_image,t_value_min)
            t_min_1 = torch.full_like(t, t_value_min-1)
            min_noise_image = self.q_sample(img,t_min_1, noise)

            if self.loss_type == 'l1':
                loss = F.l1_loss(noise, predicted_noise) +F.mse_loss(predicted_min_noise_image,min_noise_image)
            elif self.loss_type == 'l2':
                loss = F.mse_loss(noise, predicted_noise) +F.mse_loss(predicted_min_noise_image,min_noise_image)
            elif self.loss_type == "huber":
                loss = F.smooth_l1_loss(noise, predicted_noise) +F.mse_loss(predicted_min_noise_image,min_noise_image)
            else:
